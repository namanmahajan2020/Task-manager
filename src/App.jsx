    ...
    <>
      ...
      <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-1/2">
        ...
        <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <button onClick={handleAdd} disabled={todo.length < 3} className='bg-violet-800 hover:bg-violet-950 disabled:bg-violet-400 p-4 font-bold py-2 text-sm text-white rounded-full mx-2'>Save</button>
          </div>
        </div>
          ...
          {todos.map(item => {
            return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex my-3 justify-between">
              ...
              <div className="buttons flex h-full">
                ...
                <button onClick={(e) => { handleDelete(e, item.id) }} className='bg-violet-800 hover:bg-violet-950 p-2 font-bold py-1 text-sm text-white rounded-md mx-1'><AiFillDelete /></button>
              </div>
            </div>
          })}
        </div>
